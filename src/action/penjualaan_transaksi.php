...
foreach ($barang_id as $i => $id_barang) {
    ...
    $stok_result = mysqli_query($koneksi, "SELECT nama_barang, stok FROM barang WHERE id = $id_barang");
    $stok_data = mysqli_fetch_assoc($stok_result);
    $nama_barang = $stok_data['nama_barang'];
    $stok_ada = $stok_data['stok'] ?? 0;

    if ($stok_ada < $jumlah) {
        mysqli_query($koneksi, "DELETE FROM penjualan WHERE id = $id_penjualan");

        $pesan = urlencode("Stok tidak mencukupi untuk $nama_barang. Tersisa $stok_ada");
        header("Location: ../index.php?page=penjualan&error=$pesan");
        exit;
    }

    mysqli_query($koneksi, "INSERT INTO detail_penjualan (id_penjualan, id_barang, qty) VALUES ('$id_penjualan', '$id_barang', '$jumlah')");
...
header("Location: ../index.php?page=laporan&success=$pesan");
exit;
?>
